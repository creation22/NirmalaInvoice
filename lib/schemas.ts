...
const UpiQrPartSchema = z.object({
    partNumber: z.number(),
...
const PaymentInformationSchema = z
    .object({
        bankName: fieldValidators.string.optional().default(""),
        accountName: fieldValidators.string.optional().default(""),
        accountNumber: fieldValidators.string.optional().default(""),
        upiEnabled: z.boolean().optional().default(false),
        upiId: fieldValidators.stringOptional,
        qrParts: z.array(UpiQrPartSchema).optional(),
        // Optional so invoices saved before cap-based QR splits still parse.
        upiInstallments: z.number().optional(),
        installmentDetails: z.array(z.unknown()).optional(),
    })
...
const InvoiceDetailsSchema = z.object({
    ...
    // India-only: keep the field so saved invoices still parse, but always INR.
    currency: fieldValidators.string.default("INR").transform(() => "INR"),
...
export { InvoiceSchema, ItemSchema, UpiQrPartSchema };
