...
/**
 * India-only money display. Uses the rupee sign and Indian grouping
 * (e.g. ₹1,00,000.00) rather than an ISO code like INR or USD.
 */
const formatRupees = (value: unknown): string => {
    const amount = Number(value ?? 0);
    return `₹${formatNumberWithCommas(Number.isFinite(amount) ? amount : 0)}`;
};
