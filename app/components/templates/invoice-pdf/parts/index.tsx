...
// Helpers
import { formatDate, formatRupees, isDataUrl } from "@/lib/helpers";
import {
    buildUpiUri,
    qrDataUrl,
    resolveUpiQrParts,
 ...
 *
 * nirmalaUPI is India-only, so invoices always show ₹ with Indian grouping
 * (₹1,00,000.00) rather than an ISO code. The previous `currencyDisplay: "code"`
 * path existed to survive 170 currencies in embedded PDF fonts; that is no
 * longer needed. ₹ falls back per-character to IBM Plex Sans / system-ui.
 */
const money = (value: unknown) => formatRupees(value);
...
export function TotalsBlock({
    ...
    const rows: [string, string][] = [
        [labels.subtotal, money(details.subTotal)],
    ];
    ...
    if (details.discountDetails?.amount) {
        rows.push([
            ...
            details.discountDetails.amountType === "amount"
                ? `- ${money(details.discountDetails.amount)}`
    ...
    if (details.taxDetails?.amount) {
        rows.push([
            ...
            details.taxDetails.amountType === "amount"
                ? `+ ${money(details.taxDetails.amount)}`
    ...
    if (details.shippingDetails?.cost) {
        rows.push([
            ...
            details.shippingDetails.costType === "amount"
                ? `+ ${money(details.shippingDetails.cost)}`
        ...
        <div className="w-full">
            ...
            {details.totalAmountInWords && (
                <p className={`${scale.label} mt-1.5 italic text-gray-500`}>
                    {labels.totalInWords}: {details.totalAmountInWords} Rupees
                </p>
            )}
        </div>
    );
...
export function PaymentBlock({ ctx }: { ctx: PartCtx }) {
    ...
    const upiOn = Boolean(pay?.upiEnabled);

    if (upiOn) {
        const upiId = pay?.upiId?.trim() ?? "";
        const payeeName = data.sender?.name || "Merchant";
        const invoiceNo = data.details.invoiceNumber || "invoice";
        const qrParts = resolveUpiQrParts({
            upiEnabled: true,
            totalAmount: data.details.totalAmount,
        });
        const count = qrParts.length;
        const layout = upiQrLayout(count);
        const totalBill = data.details.totalAmount;

        return (
            <div data-edit-field="details.paymentInformation.upiId">
                <div className="border border-gray-300 bg-white">
                    <div className="flex items-end justify-between gap-4 border-b border-gray-200 px-5 py-3">
                        <div>
                            <p
                                className={`${scale.label} font-semibold uppercase tracking-wider text-gray-500`}
                            >
                            ...
                            {upiId ? (
                                <p
                                    className={`${scale.body} mt-1 font-semibold text-gray-900`}
                                >
                                    UPI ID: {upiId}
                                </p>
                            ) : (
                                <p className={`${scale.body} mt-1 text-gray-500`}>
                                    Add a UPI ID to generate payment QR codes.
                                </p>
                            )}
                        </div>
                        {upiId && (
                            <p
                                className={`${scale.total} shrink-0 font-bold tabular-nums text-gray-900`}
                            >
                    ...
                    {upiId && (
                        <>
                            <div className={layout.wrap}>
                                {qrParts.map((item) => {
                                    const note = `Invoice ${invoiceNo} · QR ${item.partNumber}/${count}`;
                                    ...
                                    const qr = (
                                        <img
                                            src={qrDataUrl(uri, layout.size)}
                                            alt={`UPI QR ${item.partNumber} of ${count}`}
                                            width={layout.size}
                                            height={layout.size}
                                    ...
                                    const label = (
                                        <>
                                            <p
                                                className={`${scale.label} font-semibold uppercase tracking-wide text-gray-600`}
                                            >
                                                QR {item.partNumber}
                                                {count > 1 ? ` of ${count}` : ""}
                                            </p>
                                            <p
                                                className={`${scale.total} mt-0.5 font-bold tabular-nums text-gray-900`}
                                            >
                                                {money(item.amount)}
                                            </p>
                                        </>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div>
                                                    {label}
                                                    <p
                                                        className={`${scale.body} mt-2 text-gray-600`}
                                                    >
                            ...
                            <p
                                className={`${scale.body} border-t border-gray-200 px-5 py-3 text-center text-gray-600`}
                            >
                                {count > 1
                                    ? `All QR codes together equal the total bill of ${money(totalBill)}.`
                                    : `This QR equals the total bill of ${money(totalBill)}.`}
                            </p>
                        </>
                    )}
                </div>
