...
const WizardProgress = () => {
    ...
    const steps = useMemo(() => {
        ...
        return WIZARD_STEPS.map((step) => {
            ...
            if (isPaymentStep(step.id)) {
                const upiOn = Boolean(
                    values["details.paymentInformation.upiEnabled"]
                );
                const isComplete = upiOn
                    ? filled(values["details.paymentInformation.upiId"])
                    : [
                          "details.paymentInformation.bankName",
                          "details.paymentInformation.accountName",
                          "details.paymentInformation.accountNumber",
                      ].every((field) => filled(values[field]));
                ...
                return {
                    ...
                    isStarted:
                        isComplete ||
                        filled(values["details.paymentInformation.upiId"]) ||
                        filled(values["details.paymentInformation.bankName"]),
                };
            }
